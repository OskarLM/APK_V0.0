...
if (window.__APP_LOADED__) {
  ...
  function _recentrarCasa(container, btnLeft, btnCenter, btnRight) {
    ...
    requestAnimationFrame(() => {
      ...
      requestAnimationFrame(() => {
        ...
        let rightRect; const visible = !!(btnRight && btnRight.style.display !== 'none' && btnRight.style.pointerEvents !== 'none' && btnRight.style.opacity !== '0');
        if (visible) { rightRect = btnRight.getBoundingClientRect(); }
        else { const estLeft = (typeof footerAnchors.centerX === 'number') ? footerAnchors.centerX : (container.clientWidth / 2) - (c.width / 2); rightRect = { left: fr.left + estLeft, width: c.width }; }
  ...
  function getBalanceRightOffset(container){ try{ const resetBtn = Array.from(document.querySelectorAll('.footer-controles .btn-small')).find(b => /reset/i.test((b.textContent||'').trim())); if (!resetBtn || !container) return 20; const contRect = container.getBoundingClientRect(); const resetRect = resetBtn.getBoundingClientRect(); return Math.max(0, Math.round(contRect.right - resetRect.right)); }catch(e){ return 20; } }
  function layoutBalanceFixed(container, balanceEl){ if (!container || !balanceEl) return; const cs = getComputedStyle(container); if (cs.position === 'static') container.style.position = 'relative'; const rightOffset = getBalanceRightOffset(container); balanceEl.style.position='absolute'; balanceEl.style.right= rightOffset + 'px'; balanceEl.style.top='50%'; balanceEl.style.transform='translateY(-50%)'; }
  function layoutBalanceReset(balanceEl){ if (!balanceEl) return; balanceEl.style.position=''; balanceEl.style.right=''; balanceEl.style.top=''; balanceEl.style.transform=''; }
  ...
  function mostrar() {
    ...
    if (balanceEl){ balanceEl.textContent = t.toFixed(2) + " €"; if (t < 0) balanceEl.style.color = "var(--danger)"; else if (t <= (750 * factor)) balanceEl.style.color = "var(--warning)"; else if (t <= (1400 * factor)) balanceEl.style.color = "var(--success)"; else balanceEl.style.color = "var(--electric-blue)"; }
    const footerRow = document.querySelector('.footer-row'); const plus = ensureThreePlusButtons();
    const btnLeft = plus[0] || null; const btnCenter = plus[1] || null; const btnRight = plus[2] || null; const modo = movDiv.dataset.modo || "lista";
    ...
    if (modo === "graficos") {
      ...
      layoutFooterGrafico1(footerRow, btnLeft, btnCenter, btnRight); layoutBalanceFixed(footerRow, balanceEl);
      ...
      layoutFooterGrafico2(footerRow, btnLeft, btnCenter, btnRight); layoutBalanceFixed(footerRow, balanceEl);
    } else {
      ...
      layoutFooterReset(btnLeft, btnCenter, btnRight); layoutBalanceReset(balanceEl);
      const cont = document.querySelector('.footer-controles'); if (cont) cont.style.display = fullscreenMode ? 'none' : '';
    }
