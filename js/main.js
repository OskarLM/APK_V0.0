// === main.js v24 — UX mejorado, balance real, importes frecuentes, editar desde gráficos, notificaciones, virtualización lista, PDF nativo ===
// ==========================
// CFG CENTRALIZADO
// ==========================
const CFG = {
  ...
  VIRTUAL_THRESHOLD: 200,   // registros a partir de los cuales virtualizar
  NOTIF_CHECK_MS:    60000, // intervalo comprobación presupuesto
};
...
const showToast = (msg, type = 'ok') => {
  ...
  if (!t) {
    ...
    t.style.cssText = `position:fixed;bottom:110px;left:50%;transform:translateX(-50%) translateY(20px);background:rgba(0,0,0,.92);color:#fff;padding:12px 22px;border-radius:22px;font-size:15px;font-weight:700;z-index:99999;opacity:0;transition:opacity .3s,transform .3s;pointer-events:none;max-width:85vw;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,.5);`;
  ...
  _toastTimer = setTimeout(()=>{ t.style.opacity='0'; t.style.transform='translateX(-50%) translateY(20px)'; }, 3200);
};
...
function mostrar() {
  ...
  if(balEl){
    // Balance real del mes (ingresos - gastos, sin filtros de cat/sub/ori)
    const ahora_b=new Date();
    const mesF=fs[0],anioF=fs[1];
    const esMesFiltrado=mesF!=='TODOS'&&anioF!=='TODOS';
    let balanceReal=total;
    let labelBalance='';
    if(esMesFiltrado&&(fs[2]!=='TODAS'||fs[3]!=='TODAS'||fs[4]!=='TODOS')){
      // Hay filtros adicionales: mostrar balance real del mes aparte
      const todosMes=movimientos.filter(m=>{const dv=(m.f||'').split('-');return(parseInt(dv[1])-1).toString()===mesF&&dv[0]===anioF;});
      balanceReal=todosMes.reduce((s,m)=>s+Number(m.imp),0);
      labelBalance=' ⚠️';
    }
    balEl.textContent=balanceReal.toFixed(2)+' €'+labelBalance;
    balEl.title=labelBalance?'Balance real del mes (ignorando filtros de categoría)':'';
    balEl.style.color=balanceReal<0?'var(--danger)':balanceReal<=CFG.BALANCE_WARNING*factor?'var(--warning)':balanceReal<=CFG.BALANCE_OK*factor?'var(--success)':'var(--electric-blue)';
    ...
    if(modoInicio==='dashboard'&&movimientos.length>0){
      ...
      if(listaDiv){
        if(_modoMasivo){
          ...
          if(filtradosGlobal.length>CFG.VIRTUAL_THRESHOLD){
            // Virtualización simple: renderizar solo visible + buffer
            const ITEM_H=85,visibles=Math.ceil(window.innerHeight/ITEM_H)+10;
            const scrollTop=listaDiv.scrollTop||window.scrollY;
            const startIdx=Math.max(0,Math.floor(scrollTop/ITEM_H)-5);
            const endIdx=Math.min(filtradosGlobal.length,startIdx+visibles+5);
            const paddingTop=startIdx*ITEM_H,paddingBot=(filtradosGlobal.length-endIdx)*ITEM_H;
            listaDiv.innerHTML=slice.length
              ?`<div style="height:${paddingTop}px"></div>`+filtradosGlobal.slice(startIdx,endIdx).map(renderCard).join('')+`<div style="height:${paddingBot}px"></div>`
              :`<div class="card" style="text-align:center;border:none;opacity:.7">No hay registros.</div>`;
          }else{
            listaDiv.innerHTML=slice.length?slice.map(renderCard).join(''):`<div class="card" style="text-align:center;border:none;opacity:.7">No hay registros.</div>`;
          }
...
const abrirDetalleMovs=(categoria,subcategoria)=>{
  try{
    ...
    overlay.innerHTML=`<div class="premium-content" style="max-height:80vh;overflow:auto;text-align:left">
      ...
      ${lista.length?lista.map(m=>`<div class="card" style="margin:10px 0;border-left-color:var(--danger);cursor:pointer" onclick="document.querySelector('.premium-overlay').remove();abrirFormulario('${m.id}')">
        <div class="meta">${esc(m.f.split('-').reverse().join('/'))} • ${esc(m.o)}</div>
        ${m.d?`<div style="font-size:13px;opacity:.9;margin-bottom:6px">${esc(m.d)}</div>`:''}
        <div style="display:flex;justify-content:space-between;align-items:center">
          <div class="monto" style="position:static;transform:none;color:var(--danger)">${Math.abs(m.imp).toFixed(2)} €</div>
          <span style="font-size:11px;color:var(--primary);opacity:.7">✏️ editar</span>
        </div>
      </div>`).join(''):`<div class="card" style="text-align:center;border:none;opacity:.8">No hay movimientos.</div>`}
...
const guardar=()=>{
  ...
  // Feedback visual grande al guardar
  const msgGuardar=v.editId?'✏️ Actualizado':'✅ Guardado — '+v.categoria+' '+Math.abs(imp).toFixed(2)+'€';
  showToast(msgGuardar,'ok');
  // Feedback adicional: vibración táctil si disponible
...
const init=()=>{
  ...
  checkResumenSemanal();
  // Notificaciones: pedir permiso una vez todo cargado
  setTimeout(initNotificaciones, 1000);
};
...
// ==========================
// WEB NOTIFICATIONS — presupuesto superado
// ==========================
...
const enviarNotificacion=(titulo,cuerpo,icono='')=>{
  if(Notification.permission!=='granted')return;
  try{new Notification(titulo,{body:cuerpo,icon:icono||'/APK_V0.0/icons/icon-192.png',badge:'/APK_V0.0/icons/icon-192.png',tag:'presupuesto-'+titulo});}catch(e){}
};
const checkNotificacionesPresupuesto=()=>{
  if(!Object.keys(presupuestos).length)return;
  const ahora=new Date();
  const stats=calcEstadisticasMes(ahora.getFullYear(),ahora.getMonth());
  for(const[cat,lim]of Object.entries(presupuestos)){
    const gast=stats.porCat[cat]||0;
    if(gast>=lim){enviarNotificacion('⚠️ Presupuesto superado',`${cat}: ${Math.round(gast)}€ / ${lim}€`);}
    else if(gast>=lim*0.9){enviarNotificacion('⚡ Casi sin presupuesto',`${cat}: ${Math.round(gast)}€ / ${lim}€ (${Math.round((gast/lim)*100)}%)`);}
  }
};
// Pedir permiso al desbloquear, comprobar cada minuto
const initNotificaciones=async()=>{
  await pedirPermisoNotificaciones();
  setInterval(checkNotificacionesPresupuesto,CFG.NOTIF_CHECK_MS);
};
...
let _syncTimer=null;
const scheduleSync=(reason='changed')=>{clearTimeout(_syncTimer);_syncTimer=setTimeout(async()=>{try{if(navigator.onLine){const ok=await _dbxUpload(await encryptBackup(buildBackupObject()));if(ok){const ind=document.getElementById('backupIndicator');if(ind){ind.querySelector('.txt').textContent='☁️ Sincronizado';setTimeout(updateBackupIndicator,3000);}}}}catch{}},CFG.SYNC_DEBOUNCE_MS);};
...
Object.assign(window,{
  ...
  calcImporteFrecuente,isCasaCategory,calcEstadisticasMes,
  pedirPermisoNotificaciones,checkNotificacionesPresupuesto,
});
