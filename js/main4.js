// === main.js v19 — Refactorizado: scroll infinito corregido, Dropbox deduplicado, localStorage seguro ===
if (window.__APP_LOADED__) {
  ...
  const lsSet    = (k, v)         => { try { localStorage.setItem(k, JSON.stringify(v)); } catch (e) { console.warn('localStorage lleno:', e); } };
  ...
  const ensureDefaultPinHash = async () => {
    if (!lsRaw(PIN_STORAGE_KEY)) lsRawSet(PIN_STORAGE_KEY, await sha256("7143"));
  };
  ...
  const buildBackupObject = () => ({
    meta:  { createdAt: new Date().toISOString(), app: "mis-gastos", version: "V1.0.27" },
  ...
  const verifyAndUnlock = async (pinPlain) => {
    ...
    if (remainMs > 0) { alert(`Has superado el número de intentos. Espera ${Math.ceil(remainMs/1000)} s.`); return; }
    await ensureDefaultPinHash();
    if (await sha256(pinPlain) === lsRaw(PIN_STORAGE_KEY)) {
      ...
      if (prev >= 5) { setCooldown(60); setAttempts(0); alert("Demasiados intentos. Bloqueo de 60 s."); }
  ...
  const pressPin = async (n) => {
    ...
    if (remain > 0) { alert(`Bloqueado. Espera ${Math.ceil(remain/1000)} s.`); return; }
  ...
  function mostrar() {
    ...
    if (balanceEl) {
      ...
      balanceEl.style.color = total < 0 ? "var(--danger)"
        : total <= 750*factor  ? "var(--warning)"
        : total <= 1400*factor ? "var(--success)"
        : "var(--electric-blue)";
    ...
    if (modo === "graficos") {
      ...
      if (listaDiv) {
        listaDiv.innerHTML = filtradosGlobal.slice(0, registrosVisibles).map(m => `
  ...
  const renderizarBarrasGraficos = (f) => {
    ...
    lista.innerHTML = html + items.map(([label, val]) => {
      ...
      const t2 = val > 50*f  ? Math.min(val-50*f,  150*f) : 0;
      const t3 = val > 200*f ? Math.min(val-200*f, 300*f) : 0;
      const t4 = val > 500*f ? val-500*f : 0;
      return `
  ...
  const renderizarGraficos2 = () => {
    ...
    const colorMes = (t) => t < 0 ? "var(--danger)" : t <= 250 ? "var(--warning)" : t <= 750 ? "var(--success)" : "var(--electric-blue)";
  ...
  const abrirFormulario = (id = null) => {
    ...
    if (id) {
      ...
      onOrigenChange(document.getElementById("origen").value || "");
  ...
  const guardar = () => {
    ...
    if (!v.origen || !v.categoria || !v.subcategoria || !v.fecha || isNaN(imp)) {
      alert("Faltan datos (revisa Origen/Categoría/Subcategoría/Fecha e Importe)."); return;
    }
    const m = {
  ...
  const manejarNuevo = (el, tipo) => {
    ...
    if (tipo === "categoria") {
      if (NOMINA_CATS.some(x => canonicalizeLabel(x) === keyNew)) { alert("No puedes crear manualmente 'Oskar' ni 'Josune'."); el.value = ""; return; }
  ...
  const borrarElemento = (tipo) => {
    ...
    if (tipo === 'categoria') {
      ...
      if (idx < 0) { alert('Solo puedes borrar categorías añadidas por ti.'); return; }
  ...
  const bindInfiniteScroll = () => {
    ...
    window.addEventListener('scroll', () => {
      ...
      if (window.scrollY + window.innerHeight < document.documentElement.scrollHeight - 300) return;
  ...
  // ==========================
  const exportarCSV = () => {
    if (!movimientos.length) { alert("No hay datos para exportar."); return; }
    const SEP  = ";";
  ...
  // Upload centralizado — usado por sync manual y auto‑sync
  ...
  const dropboxUploadEncryptedBackup = async () => {
    try {
      ...
      alert(ok ? '✅ Copia subida a Dropbox.' : '❌ Error al subir a Dropbox.');
  ...
  const dropboxSignOut = () => { dbx_clearTokens(); alert('Dropbox desconectado en este dispositivo.'); };
  ...
  // ==========================
  // EXPORTAR A GLOBAL (para HTML inline handlers)
  // ==========================
