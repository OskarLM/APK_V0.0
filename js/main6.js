// === main.js v21 — Inteligencia datos, Insight mensual, Heatmap, Presupuestos, Cambio PIN, Borrado seguro, Voz ===
if (window.__APP_LOADED__) {
  ...
  let presupuestos= lsGet('presupuestos_v1', {}); // { "Comida": 300, "Gasolina": 150, ... }
  ...
  const isoToYYYYMM = (iso) => (iso||'').slice(0,7);
  const hoy = () => new Date();
  ...
  const buildBackupObject = () => ({ meta:{createdAt:new Date().toISOString(),app:'mis-gastos',version:'V1.0.29'}, datos:{movimientos,catExtra,subMaestra,presupuestos} });
  ...
  const pressPin = async (n) => {
    const rem=isInCooldown(); if(rem>0){showToast(`Bloqueado ${Math.ceil(rem/1000)} s`,'err');return;}
    if(pinActual.length<4){ pinActual+=String(n); updateDots(); if(pinActual.length===4){const c=pinActual;clearPin();await ensureDefaultPinHash();verifyAndUnlock(c);} }
  };

  const biometricAuth = async () => showToast('Biometría no disponible aún','warn');

  // Cambio de PIN desde la app (sin tocar código)
  const cambiarPin = async () => {
    ...
    overlay.innerHTML = `
      <div class="premium-content" style="text-align:center">
        ...
        <p style="opacity:.7;font-size:13px;margin-bottom:16px">Introduce tu PIN actual y el nuevo (4 dígitos)</p>
        <input id="_pinOld" type="password" inputmode="numeric" maxlength="4" placeholder="PIN actual"
        ...
        <input id="_pinNew1" type="password" inputmode="numeric" maxlength="4" placeholder="Nuevo PIN"
          style="width:100%;padding:10px;border-radius:12px;border:1px solid rgba(212,175,55,.4);background:rgba(255,255,255,.07);color:inherit;font-size:18px;text-align:center;margin-bottom:10px;box-sizing:border-box">
        <input id="_pinNew2" type="password" inputmode="numeric" maxlength="4" placeholder="Repetir nuevo PIN"
    ...
    overlay.querySelector('#_pinCambiarOk').onclick = async () => {
      ...
      if(!/^\d{4}$/.test(new1)){ showToast('El PIN debe tener 4 dígitos','err'); return; }
      await ensureDefaultPinHash();
  ...
  // Borrado seguro con confirmación doble
  const borradoSeguro = () => {
    const overlay = document.createElement('div');
    overlay.className = 'premium-overlay';
    overlay.innerHTML = `
      <div class="premium-content" style="text-align:center">
        ...
        <p style="opacity:.8;margin-bottom:10px">Esto eliminará <b>todos los datos</b> de la app de este dispositivo. No se puede deshacer.</p>
    ...
    overlay.querySelector('#_borrarOk').onclick = () => {
      const val = overlay.querySelector('#_confirmBorrar').value.trim().toUpperCase();
      if(val !== 'BORRAR'){ showToast('Escribe exactamente: BORRAR','err'); return; }
      ['movimientos','categoriaExtra','subMaestra_v2','presupuestos_v1','pin_hash_v1','pin_attempts_v1',
       'pin_cooldown_until','pin_initialized','backup_last_ts','backup_idx','dbx_tokens',
       'backup_1','backup_2','backup_3','backup_4','backup_5'].forEach(k=>{ try{localStorage.removeItem(k);}catch{} });
      overlay.remove();
      showToast('Datos borrados. Recargando...','warn');
  ...
  const unlock = () => {
    ...
    const m=document.getElementById('movimientos');
    m.classList.remove('hidden'); m.dataset.permiso='OK';
  ...
  // ==========================
  // VOZ — dictado de gastos
  // ==========================
  let _recognizing = false;
  const iniciarDictado = () => {
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
    if(!SR){ showToast('Reconocimiento de voz no disponible en este navegador','warn'); return; }
    ...
    _recognizing = true;
    showToast('🎤 Escuchando... di el importe y categoría','ok');
    r.start();
    r.onresult = (e) => {
      _recognizing = false;
      const transcript = e.results[0][0].transcript.toLowerCase().trim();
      showToast(`🎤 "${transcript}"`, 'ok');
      _procesarDictado(transcript);
    };
    r.onerror = (e) => { _recognizing=false; showToast('Error de voz: '+e.error,'err'); };
    r.onend   = () => { _recognizing=false; };
  };

  const _procesarDictado = (texto) => {
    // Extraer importe: busca números con posible "euros", "€", coma/punto decimal
    const matchNum = texto.match(/(\d+(?:[.,]\d{1,2})?)\s*(?:euros?|€)?/i);
    if(!matchNum){ showToast('No entendí el importe. Inténtalo de nuevo.','warn'); return; }
    const imp = parseFloat(matchNum[1].replace(',','.'));
    if(isNaN(imp)){ showToast('Importe no reconocido','warn'); return; }

    // Buscar categoría/subcategoría mencionada
    ...
    setTimeout(()=>{
      ...
      showToast(`✅ ${imp}€${catFound?' · '+catFound:''}${subFound?' · '+subFound:''}`, 'ok');
    }, 300);
  };
  ...
  const calcEstadisticasMes = (yearNum, monthNum) => {
    ...
    const gastosMes = movimientos.filter(m=>{
      const d=(m.f||'').split('-');
      return parseInt(d[0])===yearNum && parseInt(d[1])-1===monthNum && m.imp<0;
    });
    const total = gastosMes.reduce((s,m)=>s+Math.abs(m.imp),0);
    const porCat = {};
    for(const m of gastosMes) porCat[m.c]=(porCat[m.c]||0)+Math.abs(m.imp);
    return { total, porCat, n: gastosMes.length };
  };
  ...
  const calcPromedioHistorico = (cat, mesesAtras=6) => {
    ...
    for(let i=1;i<=mesesAtras;i++){
      ...
      const stats=calcEstadisticasMes(d.getFullYear(),d.getMonth());
      if(stats.porCat[cat]){ suma+=stats.porCat[cat]; count++; }
    }
  ...
  const generarInsightMensual = () => {
    ...
    const mesAnteriorStats=calcEstadisticasMes(añoActual,mesActual-1<0?11:mesActual-1, mesActual-1<0?añoActual-1:añoActual);

    // Predicción fin de mes (ratio días)
    const prediccion = diaHoy>0 ? Math.round((stats.total/diaHoy)*diasMes) : 0;

    // Diferencia vs mes anterior
    const diff = stats.total - mesAnteriorStats.total;
    const diffPct = mesAnteriorStats.total>0 ? Math.round((diff/mesAnteriorStats.total)*100) : 0;

    // Anomalías: categorías que superan >60% su promedio
    const anomalias=[];
    for(const [cat,val] of Object.entries(stats.porCat)){
      const prom=calcPromedioHistorico(cat,6);
      if(prom>20 && val>prom*1.6) anomalias.push({cat,val,prom,ratio:Math.round((val/prom)*100)});
    }
    anomalias.sort((a,b)=>b.ratio-a.ratio);

    // Cat más cara este mes
    const topCat = Object.entries(stats.porCat).sort((a,b)=>b[1]-a[1])[0];

    return { prediccion, diff, diffPct, anomalias: anomalias.slice(0,2), topCat, total: stats.total, diaHoy, diasMes };
  };
  ...
  const renderInsightPanel = () => {
    ...
    let html=`<div style="background:linear-gradient(135deg,rgba(212,175,55,.12),rgba(0,0,0,.3));border:1px solid rgba(212,175,55,.3);border-radius:16px;padding:16px;margin-bottom:16px;">
      <div style="font-size:13px;font-weight:700;color:var(--primary);letter-spacing:1px;margin-bottom:12px">💡 INSIGHT DE ${mesNombre.toUpperCase()}</div>`;

    // Gasto actual + predicción
    html+=`<div style="display:flex;justify-content:space-between;margin-bottom:8px">
      <div><div style="font-size:11px;opacity:.6">GASTADO</div><div style="font-size:20px;font-weight:900;color:var(--danger)">${fmtE(ins.total)}</div></div>
      <div style="text-align:right"><div style="font-size:11px;opacity:.6">PREVISIÓN FIN MES</div><div style="font-size:20px;font-weight:900;color:var(--warning)">${fmtE(ins.prediccion)}</div></div>
    </div>`;

    // Barra de progreso días
    const pctDias=Math.round((ins.diaHoy/ins.diasMes)*100);
    html+=`<div style="margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;font-size:11px;opacity:.6;margin-bottom:4px">
        <span>Día ${ins.diaHoy} de ${ins.diasMes}</span><span>${pctDias}% del mes</span>
      </div>
      <div style="height:4px;background:rgba(255,255,255,.1);border-radius:4px;overflow:hidden">
        <div style="width:${pctDias}%;height:100%;background:var(--primary);border-radius:4px"></div>
      </div>
    </div>`;

    // Vs mes anterior
    if(ins.diff!==0){
      const col=ins.diff>0?'var(--danger)':'var(--success)';
      const arrow=ins.diff>0?'↑':'↓';
      html+=`<div style="font-size:13px;margin-bottom:8px;color:${col}">${arrow} ${Math.abs(ins.diffPct)}% vs mes anterior (${ins.diff>0?'+':''}${fmtE(ins.diff)})</div>`;
    }

    // Top categoría
    if(ins.topCat) html+=`<div style="font-size:13px;opacity:.8;margin-bottom:8px">📌 Mayor gasto: <b>${ins.topCat[0]}</b> — ${fmtE(ins.topCat[1])}</div>`;

    // Anomalías
    for(const a of ins.anomalias){
      html+=`<div style="font-size:12px;color:var(--warning);margin-bottom:4px">⚠️ <b>${a.cat}</b> lleva ${fmtE(a.val)} (${a.ratio}% sobre tu media de ${fmtE(a.prom)})</div>`;
    }

    // Presupuestos con progreso
    const presKeys=Object.keys(presupuestos);
    if(presKeys.length){
      html+=`<div style="margin-top:10px;font-size:11px;font-weight:700;color:var(--primary);letter-spacing:1px;margin-bottom:6px">PRESUPUESTOS</div>`;
      const stats=calcEstadisticasMes(ahora.getFullYear(),ahora.getMonth());
      for(const cat of presKeys){
        const lim=presupuestos[cat], gast=stats.porCat[cat]||0, pct=Math.min(100,Math.round((gast/lim)*100));
    ...
    html+=`<div style="display:flex;gap:8px;margin-top:12px">
      <button onclick="abrirPresupuestos()" class="btn-silver" style="flex:1;font-size:12px;padding:7px">📊 Presupuestos</button>
      <button onclick="iniciarDictado()" class="btn-silver" style="flex:1;font-size:12px;padding:7px">🎤 Voz</button>
      <button onclick="cambiarPin()" class="btn-silver" style="flex:1;font-size:12px;padding:7px">🔑 PIN</button>
    </div></div>`;
  ...
  const abrirPresupuestos = () => {
    ...
    const rows=cats.map(c=>`
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
        ...
        <input type="number" min="0" placeholder="0" value="${presupuestos[c]||''}"
          data-cat="${esc(c)}"
    ...
    overlay.innerHTML=`
      <div class="premium-content" style="max-height:75vh;overflow:auto">
    ...
    overlay.querySelector('#_presSave').onclick=()=>{
      const nuevos={};
      overlay.querySelectorAll('input[data-cat]').forEach(inp=>{
        const v=parseFloat(inp.value); if(v>0) nuevos[inp.dataset.cat]=v;
      });
  ...
  const renderHeatmap = () => {
    const lista=document.getElementById('lista'); if(!lista) return;
    // Últimas 12 semanas — qué día de la semana se gasta más
    const ahora=new Date();
    const desde=new Date(ahora); desde.setDate(desde.getDate()-83);
    const porDia=[0,0,0,0,0,0,0]; // Dom=0 … Sáb=6
    const countDia=[0,0,0,0,0,0,0];
    for(const m of movimientos){
      if(m.imp>=0) continue;
      const d=new Date(m.f+'T00:00:00');
      if(d<desde) continue;
      const wd=d.getDay();
      porDia[wd]+=Math.abs(m.imp);
      countDia[wd]++;
    }
    const labDias=['Dom','Lun','Mar','Mié','Jue','Vie','Sáb'];
    const maxD=Math.max(...porDia,1);
    const colorH=(v)=>{ const pct=v/maxD; return pct>0.8?'var(--danger)':pct>0.5?'var(--warning)':pct>0.2?'var(--success)':'rgba(212,175,55,.2)'; };
    ...
    for(let i=1;i<=7;i++){
      const wd=i%7;
      const v=porDia[wd], avg=countDia[wd]>0?Math.round(v/countDia[wd]):0;
      const h=Math.max(8,Math.round((v/maxD)*50));
      html+=`<div>
        <div style="height:${h}px;background:${colorH(v)};border-radius:4px;margin-bottom:4px"></div>
        <div style="font-size:10px;opacity:.7">${labDias[wd]}</div>
        <div style="font-size:9px;opacity:.5">${avg}€</div>
      </div>`;
    }
    html+=`</div></div>`;
    lista.insertAdjacentHTML('afterbegin', html);
  ...
  function mostrar() {
    ...
    if(modo==='graficos'){
      ...
      captureBalanceRef();
      // Insight en modo lista (solo si hay datos y no hay filtro activo)
  ...
  const renderizarBarrasGraficos=(f)=>{
    ...
    if(!items.length){lista.innerHTML=html+`<div class="card" style="text-align:center;border:none;opacity:.8">No hay datos para los filtros seleccionados.</div>`;return;}

    // Añadir barra de presupuesto si existe
    lista.innerHTML=html+items.map(([label,val])=>{
      ...
      return `<div class="card" style="border:none;background:transparent;cursor:pointer" data-label="${esc(label)}" onclick="handleGraficoBarClick(this.dataset.label)">
        <div style="display:flex;justify-content:space-between;font-size:14px;margin-bottom:5px">
          <span>${esc(label)}</span>
          <b>${val.toFixed(2)} €${pres?` <span style="font-size:11px;opacity:.5">/ ${pres}€</span>`:''}</b>
        </div>
  ...
  const renderizarGraficos2=()=>{
    ...
    const sumaMes=new Map(); for(const mov of base){const k=(mov.f||'').slice(0,7);if(meses.some(x=>x.key===k))sumaMes.set(k,(sumaMes.get(k)||0)+(Number(mov.imp)||0));}
    ...
    for(const m of meses){const v=sumaMes.get(m.key)||0,h=Math.max(4,(Math.abs(v)/maxAbs)*80),label=mesesC[new Date(m.key+'-01T00:00:00').getMonth()];
  ...
  const guardar=()=>{
    ...
    // Comprobar presupuesto alcanzado
    if(v.origen==='Gasto'&&presupuestos[v.categoria]){
      const ahora=new Date();
      const stats=calcEstadisticasMes(ahora.getFullYear(),ahora.getMonth());
      const gast=stats.porCat[v.categoria]||0;
      const lim=presupuestos[v.categoria];
      if(gast>=lim) showToast(`⚠️ Has superado el presupuesto de ${v.categoria} (${Math.round(gast)}€ / ${lim}€)`,'warn');
      else if(gast>=lim*0.8) showToast(`⚡ Cerca del límite en ${v.categoria}: ${Math.round(gast)}€ / ${lim}€`,'warn');
    }
